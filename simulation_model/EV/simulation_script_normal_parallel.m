...
for i = 1:length(cycle_name_list)
    ...
    in(1:temperature_range, 1:soc_range) = Simulink.SimulationInput(model);
    % for every type of anomaly
    for j = 1:temperature_range
        % for every anomaly length
        for k = 1:soc_range
            % Sample initial battery temperature from range of 10°C to 30°C
            bat_temp_init = round(unifrnd(10, 30), 3, "significant");
            bat_soc_init = round(unifrnd(0.1, 1), 3, "significant");

            anomaly_start = 0;

            % Set variables for simulation
            % for every parameter
            for l = 1:length(normal_coefficients)
                in(j, k) = in(j, k).setVariable(anomaly_type_list{l}, normal_coefficients{l});
            end

            in(j, k) = in(j, k).setVariable('bat_temp_init', bat_temp_init);
            in(j, k) = in(j, k).setVariable('bat_soc_init', bat_soc_init);
