...
#define S_FUNCTION_NAME CurrPowerLimCalc_msf
#endif
...
static void mdlPeriodicOutputUpdate ( SimStruct * S , int_T tid ) { real32_T
const * i_dn3bdtflyf = ( real32_T * ) ssGetInputPortSignal ( S , 0 ) ;
real32_T const * i_amackuu0wk = ( real32_T * ) ssGetInputPortSignal ( S , 3 )
; CurrentLimits * o_B_1_1 = ( CurrentLimits * ) ssGetOutputPortSignal ( S , 0
) ; Min_Max_Cells * o_B_1_2 = ( Min_Max_Cells * ) ssGetOutputPortSignal ( S ,
1 ) ; if ( tid == 0 ) { CurrPowerLimCalc ( i_dn3bdtflyf , i_amackuu0wk ,
o_B_1_1 , o_B_1_2 ) ; } } static void mdlInitializeSizes ( SimStruct * S ) {
...
6 , 2 ) ; if ( ! ssSetNumInputPorts ( S , 6 ) ) return ; if ( !
ssSetInputPortVectorDimension ( S , 0 , 96 ) ) return ;
...
( S , 5 , 0.0 ) ; if ( ! ssSetNumOutputPorts ( S , 2 ) ) return ; if ( !
...
SS_SIMMODE_SIZES_CALL_ONLY ) {
#if defined (MATLAB_MEX_FILE)
{ DTypeId dataTypeIdReg ; ssRegisterTypeFromNamedObject ( S , "Min_Max_Cells"
, & dataTypeIdReg ) ; if ( dataTypeIdReg == INVALID_DTYPE_ID ) return ;
...
mr_CurrPowerLimCalc_RegisterSimStateChecksum ( S ) ; ssSetNumSampleTimes ( S
...
mr_CurrPowerLimCalc_MdlInfoRegFcn ( S , "CurrPowerLimCalc" , & retVal ) ; if
( ! retVal ) return ; }
#endif
ssSetNumDWork ( S , 0 ) ; slmrRegisterPeriodicOutputUpdateMethod ( S ,
mdlPeriodicOutputUpdate ) ; ssSetSimulinkVersionGeneratedIn ( S , "23.2" ) ;
...
= 0 ; } dp1ymck0d2 ( S , slmrGetTopTidFromMdlRefChildTid ( S , 0 , false ) ,
sysRanPtr , sysTid , ( ( NULL ) ) , ( ( NULL ) ) , 0 , - 1 ) ;
ssSetModelMappingInfoPtr ( S , & ( pdpnucaqsy . rtm . DataMapInfo . mmi ) ) ;
if ( S -> mdlInfo -> genericFcn != ( NULL ) ) { _GenericFcn fcn = S ->
mdlInfo -> genericFcn ; } }
#define MDL_START
static void mdlStart ( SimStruct * S ) { } static void mdlOutputs ( SimStruct
* S , int_T tid ) { real32_T const * i_dn3bdtflyf = ( real32_T * )
ssGetInputPortSignal ( S , 0 ) ; real32_T const * i_amackuu0wk = ( real32_T *
) ssGetInputPortSignal ( S , 3 ) ; CurrentLimits * o_B_1_1 = ( CurrentLimits
* ) ssGetOutputPortSignal ( S , 0 ) ; Min_Max_Cells * o_B_1_2 = (
Min_Max_Cells * ) ssGetOutputPortSignal ( S , 1 ) ; if ( tid != CONSTANT_TID
&& tid != PARAMETER_TUNING_TID ) { CurrPowerLimCalc ( i_dn3bdtflyf ,
i_amackuu0wk , o_B_1_1 , o_B_1_2 ) ; } } static void mdlTerminate ( SimStruct
* S ) { kegbnk20qt ( ) ; return ; }
...
mr_CurrPowerLimCalc_GetDWork ( ) ; mxSetFieldByNumber ( ss , 0 , 1 , mdlrefDW
) ; } { mxArray * data = mr_CurrPowerLimCalc_GetSimStateDisallowedBlocks ( )
...
( localX , & rawData [ 0 ] , numBytes ) ; } mr_CurrPowerLimCalc_SetDWork (
