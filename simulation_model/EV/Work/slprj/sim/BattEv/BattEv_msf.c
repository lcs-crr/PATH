...
real_T rtP__BattSocInit2f2BMS_Software_DD_sldd_ ; const char *
...
& ( rtP__BattSocInit2f2BMS_Software_DD_sldd_ ) , GlobalPrm_0 , sizeof (
real_T ) ) ; } }
#endif
...
= ( real_T * ) ssGetOutputPortSignal ( S , 0 ) ; hkgv0zhr3w * localX = (
hkgv0zhr3w * ) ssGetContStates ( S ) ; if ( tid == 0 ) { BattEv ( & ( dw ->
rtm ) , i_ge3d2oowpp , i_ohnku1jn0r , o_B_1_1 , & ( dw -> rtb ) , & ( dw ->
...
mr_BattEv_RegisterSimStateChecksum ( S ) ; ssSetNumSampleTimes ( S , 3 ) ;
...
static void mdlOutputs ( SimStruct * S , int_T tid ) { lobjb0lvrdo * dw = (
lobjb0lvrdo * ) ssGetDWork ( S , 0 ) ; real_T const * i_ge3d2oowpp = ( real_T
* ) ssGetInputPortSignal ( S , 0 ) ; real_T const * i_ohnku1jn0r = ( real_T *
) ssGetInputPortSignal ( S , 1 ) ; real_T * o_B_1_1 = ( real_T * )
ssGetOutputPortSignal ( S , 0 ) ; hkgv0zhr3w * localX = ( hkgv0zhr3w * )
ssGetContStates ( S ) ; if ( tid == PARAMETER_TUNING_TID ) { BattEvTID2 ( & (
dw -> rtb ) ) ; } if ( tid != CONSTANT_TID && tid != PARAMETER_TUNING_TID ) {
if ( ssIsSampleHit ( S , 0 , tid ) || ssIsMinorTimeStep ( S ) ) { BattEv ( &
( dw -> rtm ) , i_ge3d2oowpp , i_ohnku1jn0r , o_B_1_1 , & ( dw -> rtb ) , & (
dw -> rtdw ) , localX ) ; } } }
#define MDL_UPDATE
static void mdlUpdate ( SimStruct * S , int_T tid ) { lobjb0lvrdo * dw = (
lobjb0lvrdo * ) ssGetDWork ( S , 0 ) ; c0hgp3wnut ( & ( dw -> rtdw ) ) ;
return ; }
