...
#define S_FUNCTION_NAME SOC_Estimation_msf
#endif
...
#endif
static void mdlInitializeConditions ( SimStruct * S ) { cketgpc2z3 ( ) ; }
static void mdlReset ( SimStruct * S ) { h41aqbjehq ( ) ; } static void
mdlPeriodicOutputUpdate ( SimStruct * S , int_T tid ) { real32_T const *
i_okw1fjhthx = ( real32_T * ) ssGetInputPortSignal ( S , 2 ) ; real32_T const
* i_pgdwmu1goh = ( real32_T * ) ssGetInputPortSignal ( S , 3 ) ; real32_T *
o_B_1_1 = ( real32_T * ) ssGetOutputPortSignal ( S , 0 ) ; if ( tid == 0 ) {
SOC_Estimation ( i_okw1fjhthx , i_pgdwmu1goh , o_B_1_1 ) ; f4ikgzst5j ( ) ; }
...
, 6 ) ) return ; if ( ! ssSetInputPortVectorDimension ( S , 0 , 96 ) ) return
...
( S , 5 , 0.0 ) ; if ( ! ssSetNumOutputPorts ( S , 1 ) ) return ; if ( !
...
mr_SOC_Estimation_RegisterSimStateChecksum ( S ) ; ssSetNumSampleTimes ( S ,
...
mr_SOC_Estimation_MdlInfoRegFcn ( S , "SOC_Estimation" , & retVal ) ; if ( !
retVal ) return ; }
#endif
...
= 0 ; } ekir1iaqx0 ( S , slmrGetTopTidFromMdlRefChildTid ( S , 0 , false ) ,
sysRanPtr , sysTid , ( ( NULL ) ) , ( ( NULL ) ) , 0 , - 1 ) ;
ssSetModelMappingInfoPtr ( S , & ( lgpwz0u0yd . rtm . DataMapInfo . mmi ) ) ;
...
i_okw1fjhthx = ( real32_T * ) ssGetInputPortSignal ( S , 2 ) ; real32_T const
* i_pgdwmu1goh = ( real32_T * ) ssGetInputPortSignal ( S , 3 ) ; real32_T *
o_B_1_1 = ( real32_T * ) ssGetOutputPortSignal ( S , 0 ) ; if ( tid !=
CONSTANT_TID && tid != PARAMETER_TUNING_TID ) { SOC_Estimation ( i_okw1fjhthx
, i_pgdwmu1goh , o_B_1_1 ) ; } }
#define MDL_UPDATE
static void mdlUpdate ( SimStruct * S , int_T tid ) { f4ikgzst5j ( ) ; return
; } static void mdlTerminate ( SimStruct * S ) { cefw35nic3 ( ) ; return ; }
...
mr_SOC_Estimation_GetDWork ( ) ; mxSetFieldByNumber ( ss , 0 , 1 , mdlrefDW )
; } { mxArray * data = mr_SOC_Estimation_GetSimStateDisallowedBlocks ( ) ;
...
( localX , & rawData [ 0 ] , numBytes ) ; } mr_SOC_Estimation_SetDWork (
